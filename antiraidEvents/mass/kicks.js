...
module.exports = {
    ...
    async run(client, kick) {
        ...
        if (data + 1 >= number) {
            client.util.punish(guild, executorMember, config.punish, "ShieldDefender - Anti-MassKick", `${await client.lang('antiraid.masskick.err', message.guild.id)}`, `${await client.lang('antiraid.masskick.succes', message.guild.id)}`)

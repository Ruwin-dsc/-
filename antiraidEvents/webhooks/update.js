...
module.exports = {
    ...
    async run(client, webhook) {
        ...
        if (bypass) return;
        (await guild.fetchWebhooks()).find((w) => w.channelId === webhook.target_id && w.name === webhook.changes[0].old_value.name).delete("ShieldDefender - Anti-WebhookUpdate").catch(() => { });
        client.util.punish(guild, executorMember, config.punish, "ShieldDefender - Anti-WebhookUpdate", await client.lang('antiraid.webhooksupdate.err', message.guild.id), await client.lang('antiraid.webhooksupdate.succes', message.guild.id))
    }
}
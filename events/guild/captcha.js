...
module.exports = {
    ...
    async run(client, member) {
        ...
        const message = await channel.send({ content: `${await client.lang('antiraid.captcha1', member.guild.id)} ${member.guild.name}, ${member} ${await client.lang('antiraid.captcha2', member.guild.id)}`, components: [{
            type: 1,
            components: buttons
        ...
        collector.on("collect", async (collected) => {
            if (collected.customId === captchaText) {
                ...
                message.delete();
                member.send(await client.lang('antiraid.captcha3', member.guild.id))

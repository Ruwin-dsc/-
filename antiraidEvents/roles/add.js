...
module.exports = {
  ...
  async run(client, data) {
    ...
    client.util.punish(guild, executorMember, config.punish, "ShieldDefender - Anti-RolesAdd", `${await client.lang('antiraid.roleadd.err', message.guild.id)} ${newData}`, `${await client.lang('antiraid.roleadd.succes', message.guild.id)} ${newData}`)
  }
} 
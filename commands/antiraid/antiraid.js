  ...
  async run(client, interaction) {
    ...
    collector.on("collect", async (i) => {
      if (i.customId === "menu1-antiraid") {
        ...
        const reply = await i.reply({
          content: `Configuration : ${i.values[0]}\n${await client.lang("antiraidcmd.active", interaction.guild.id)}`,
        ...
        reply.edit({
          content: `Configuration: ${i.values[0]}\n${await client.lang("antiraidcmd.sanction", interaction.guild.id)}`,
        ...
        if (param.limit) {
          reply.edit({
            content: `Configuration: ${i.values[0]}\n${await client.lang("antiraidcmd.temps", interaction.guild.id)}`,
            components: [],
          });
          ...
          limitCollector.on("collect", async (limitResponse) => {
            ...
            const time = limitResponse.content
              ...
              .split("/")[1]
              ?.toLowerCase();
            if (!number || !time) return error(await client.lang("antiraidcmd.invalide.limit", interaction.guild.id));
            if (isNaN(number))  return error(await client.lang("antiraidcmd.invalide.number", interaction.guild.id));
            if (!["h", "m", "s"].includes(time))  return error(await client.lang("antiraidcmd.invalide.time", interaction.guild.id));
    ...
    function categoriesMenu() {
      let menu = new StringSelectMenuBuilder({
        ...
        placeholder: "Category - ShieldDefender",
      });
    ...
    function subcategoriesMenu() {
      let menu = new StringSelectMenuBuilder({
        ...
        placeholder: "Subcategory - ShieldDefender",
      });

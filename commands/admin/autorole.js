    ...
    async run(client, interaction) {
        ...
        collector.on("collect", async (collected) => {
            ...
            if (value === "toggle") {
                ...
                const reply = await collected.reply({ content: await client.lang('autorole.roleselect', interaction.guild.id), components: [row, button], fetchReply: true })
                const roles = [];
                reply.createMessageComponentCollector({
                    ...
                    if (v === "roles") {
                        ...
                        if (role.managed) return response.reply({ content: await client.lang('autorole.invalderole', interaction.guild.id), ephemeral: true });
                        if (!client.util.roleManageable(interaction.member, role, interaction.guild.ownerId)) return response.reply({ content: await client.lang('autorole.erreurrole', interaction.guild.id), ephemeral: true });
                        response.deferUpdate();
                        roles.push(role.id);
                        response.message.edit({ content: `${await client.lang('autorole.roleselect', interaction.guild.id)}\n${roles.map((role) => `<@&${role}>`).join(" ,") || ""}`, components: [row, button], fetchReply: true })
                    } else if (v === "ok") {
                        config.roles = roles;

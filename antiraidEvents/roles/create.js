...
module.exports = {
    ...
    async run(client, role) {
        ...
        if (bypass) return;
        guild.roles.delete(role.role.id, "ShieldDefender - Anti-RoleCreate")

        client.util.punish(guild, executorMember, config.punish, "ShieldDefender - Anti-RoleCreate", `${await client.lang('antiraid.rolecreate.err', message.guild.id)}`, `${await client.lang('antiraid.rolecreate.succes', message.guild.id)}`)
    }
}
...
module.exports = async (interaction, embeds, ephemeral = false, util) => {
    ...
    Object.keys(embeds).forEach(async (key) => {
        embeds[key].footer = {iconURL: util.client.user.avatarURL(), text: `${await util.client.lang('footer', interaction.guild.id)}・${parseInt(key) + 1}/${embeds.length}`}
    });
